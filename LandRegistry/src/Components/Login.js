...
class Login extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (LandData) {
      ...
      this.setState({ landList: landList });
      console.log("componentDidMount is called!",landList)
      console.log("user data from methood of landlist..", this.state.landList.methods
        .getUser("0x32a9dd958f9e4b0e1aff5cbb34b4a91d1778ec7a102c5eb5e9e1d219bbcd1017").call())
    } else {
  ...
  handleSubmit = async () => {
    ...
    if (this.state.address) {
      try {
        ...
        //   .call()
        const user = await axios.post('http://localhost:3001/loginuser', data).then((response) => {
                if (response.status == 200) {
                  window.alert('Login Successful!')
                  window.localStorage.setItem('authenticated', true)
                  console.log("the responsed is.....",response.data)
